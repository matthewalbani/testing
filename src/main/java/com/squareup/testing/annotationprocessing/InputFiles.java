...
/**
 * A set of input files that are loaded from resources. These are used to seed the in-memory file
 * system with contents.
 *
 * @see FilesToProcess
 */
@Target({})
public @interface InputFiles {
  ...
  /**
   * The name of a folder that contains the files to load. For a given file name (the {@code
   * i}<sup>th</sup> file in the array returned by {@link #value()}, for example), the following is
