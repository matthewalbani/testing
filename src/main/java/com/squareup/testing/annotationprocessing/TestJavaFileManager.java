 ...
 * String, String, byte[]) createFileObject} or {@link #createJavaFileObject(JavaFileManager.Location,
 * String, JavaFileObject.Kind, byte[]) createJavaFileObject} (each of these methods is overloaded
 * to easily define either text file contents or binary file contents). Finally, the new file
 * manager can be used for new {@linkplain JavaCompiler#getTask compilation tasks}.
 *
...
public class TestJavaFileManager implements JavaFileManager {
  ...
  /**
   * A regular expression pattern for matching the end of a path. This matches the last path
   * component, after all forward slashes and up to the end of the string.
   */
  ...
  private final Map<String, TestJavaFileObject> files = Maps.newHashMap();

  /**
   * The set of files that are open for writing. These objects are the various {@link
   * OutputStream}s that are open on files in the in-memory file system. This allows the file
   * manager to flush all open files when {@link #flush()} is called.
   */
  private final Set<TestJavaFileObject> openedForWriting = Sets.newHashSet();
  ...
  /**
   * Unlinks a path from the set of in-memory files. This does not mark the associated {@link
   * FileObject} as deleted, however.
   *
   * @param filePath the path to unlink
   * @return true if deletion was successful; false otherwise (like if the named file does not
   *         exist)
  ...
  public synchronized ClassLoader getClassLoader(final Location location) {
    ...
    return new ClassLoader(parentClassLoader) {
      ...
      public Class<?> findClass(String name) throws ClassNotFoundException {
        try {
          byte classContents[] = ((TestJavaFileObject) getJavaFileForInput(location, name,
              Kind.CLASS)).getByteContents();
  ...
  public synchronized Iterable<JavaFileObject> list(Location location, String packageName,
    ...
    ImmutableList.Builder<JavaFileObject> list = ImmutableList.builder();
    for (Map.Entry<String, TestJavaFileObject> entry : files.entrySet()) {
      ...
      if (entry.getKey().startsWith(pathPrefix)
          ...
          && (recurse || END_OF_PATH.matcher(entry.getKey().substring(pathPrefix.length() + 1))
          .matches())) {
        list.add(entry.getValue());
      }
    }
    if (location == StandardLocation.CLASS_PATH) {
      for (JavaFileObject fileObj : platformFileManager.list(location, packageName, kinds,
          recurse)) {
        list.add(fileObj);
      }
    }
  ...
  /**
   * Resets the file system. Resetting the file system deletes all files and invalidates any open
   * streams for writing files. After a reset, input files will need to be re-seeded using {@link
   * #createFileObject(JavaFileManager.Location, String, String, byte[]) createFileObject} or
   * {@link #createJavaFileObject(JavaFileManager.Location, String, JavaFileObject.Kind, byte[])
   * createJavaFileObject}
   */
  public synchronized void reset() {
    openedForWriting.clear();
    // make a copy
    Iterable<? extends FileObject> filesToDelete = ImmutableList.copyOf(files.values());
