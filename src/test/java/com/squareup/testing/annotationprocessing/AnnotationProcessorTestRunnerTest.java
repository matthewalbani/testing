...
public class AnnotationProcessorTestRunnerTest {
   ...
   * but won't show other exception data (like {@link InitializationError#getCauses()} for
   * example).
   *
  ...
  private void assertNoFailures(Result result, int testCount) {
    ...
    if (!failures.isEmpty()) {
      throw consolidate(Lists.transform(failures, new Function<Failure, Throwable>() {
        @Override public Throwable apply(Failure failure) {
